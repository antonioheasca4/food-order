...
// Definirea opțiunilor pentru fiecare categorie militară
const militaryCategoryOptions = {
  // Gamelă Verde (fostă Salad)
  "Gamelă Verde": [
    {
      type: "dressing",
      name: "Dressing",
  ...
  // Rații de Campanie (fostă Sandwich)
  "Rații de Campanie": [
    {
      type: "bread",
      name: "Tip pâine",
  ...
  // Pentru Vegetarieni (fostă Pure Veg)
  "Pentru Vegetarieni": [
    {
      type: "spice_level",
...
async function updateMilitaryOptions() {
  try {
    // Conectare la baza de date
    console.log('Conectare la baza de date...');
    await connectDB();
    
    // Obține produsele din categoriile militare
    ...
    console.log(`S-au găsit ${products.length} produse cu categorii militare în baza de date.`);
    ...
    for (const product of products) {
      ...
      if (militaryCategoryOptions[category]) {
        // Actualizează opțiunile produsului
        await foodModel.findByIdAndUpdate(product._id, { 
          options: militaryCategoryOptions[category] 
        });
        
        console.log(`Opțiuni actualizate pentru produsul "${product.name}" (categoria ${category}).`);
        updatedCount++;
      } else {
        console.log(`Nu s-au găsit opțiuni pentru produsul "${product.name}" (categoria ${category}).`);
      }
    }
